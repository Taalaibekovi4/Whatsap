...
let compression = null; try { compression = require("compression"); } catch {}
...
app.use(cors({ origin: FRONT_ORIGIN, credentials: true, methods: ["GET","POST","PATCH","OPTIONS"] }));
app.options("*", cors());
app.use("/uploads", express.static(UPLOADS));

const server = http.createServer(app);
const io = new Server(server, { cors: { origin: FRONT_ORIGIN, methods: ["GET","POST","PATCH"] } });

const upload = multer({ dest: UPLOADS, limits: { fileSize: 25 * 1024 * 1024 } });

// ===== utils =====
const chatsFile = () => path.join(DATA_PATH, "chats.json");
const msgFile = (chatId) => path.join(
  DATA_PATH,
...
const atomicWrite = (file, obj) => { const tmp = file + ".tmp"; fs.writeFileSync(tmp, JSON.stringify(obj, null, 2)); fs.renameSync(tmp, file); };
const nowSec = () => Math.floor(Date.now()/1000);

const sortChats = (list) => (list || []).slice().sort((a, b) => {
  if (!!b.pinned - !!a.pinned) return !!b.pinned - !!a.pinned;
  if (!!b.unreadCount - !!a.unreadCount) return !!b.unreadCount - !!a.unreadCount;
  const at = Number(a.lastTs || 0), bt = Number(b.lastTs || 0);
...
const normalizeTarget = ({ chatId, to }) => chatId ? sanitizeId(chatId) : (to ? sanitizeId(to) : null);
...
const mapMessage = (m, chatId) => {
  ...
  if (m.type === "location" && m.location) base.location = { latitude: m.location.latitude, longitude: m.location.longitude, description: m.location.description || "" };
  if ((m.type === "vcard" || m.type === "contact" || m.type === "contact_card") && Array.isArray(m.vCards) && m.vCards.length) { base.vcard = m.vCards[0]; base.type = "vcard"; }
...
if (!fs.existsSync(ANALYTICS_FILE)) {
  try { fs.writeFileSync(ANALYTICS_FILE, JSON.stringify({ events: [], savedAt: Date.now() }, null, 2)); } catch {}
}
const readAnalytics = () => safeRead(ANALYTICS_FILE, { events: [], savedAt: 0 });
const sameMonth = (a, b) => { const A = new Date(a*1000), B = new Date(b*1000); return A.getFullYear()===B.getFullYear() && A.getMonth()===B.getMonth(); };
const pushEventUniqueMonthly = (type, chatId, ts) => {
  try {
    const a = readAnalytics();
    const t = Number(ts || nowSec());
    const dup = (a.events || []).some(ev => ev.type===type && ev.chatId===chatId && sameMonth(ev.ts, t));
    if (dup) return;
...
if (!fs.existsSync(GREET_FILE)) {
  try { fs.writeFileSync(GREET_FILE, JSON.stringify({ byChat: {}, savedAt: Date.now() }, null, 2)); } catch {}
}
...
const lastGreetTs = (chatId) => Number(readGreet().byChat?.[String(chatId)] || 0);
const GREET_COOLDOWN_SEC = Number(process.env.GREET_COOLDOWN_SEC || 3 * 60 * 60);
...
const upsertChatLast = (target, lastVal, ts) => {
  ...
  if (ix >= 0) {
    arr[ix].last = lastVal;
    arr[ix].lastTs = ts;
    if (!arr[ix].status) arr[ix].status = arr[ix].isLead === true ? "lead" : "client";
    atomicWrite(chatsFile(), { results: sortChats(arr), savedAt: Date.now() });
    io.emit("chat-patch", { id: target, last: arr[ix].last, lastTs: arr[ix].lastTs, status: arr[ix].status, isLead: arr[ix].isLead });
  } else {
    const rec = { id: target, name: `+${String(target).replace(/\D/g, "")}`, isGroup: false, unreadCount: 0, pinned: false, archived: false, last: lastVal, lastTs: ts, isLead: true, status: "lead" };
    atomicWrite(chatsFile(), { results: sortChats([rec, ...arr]), savedAt: Date.now() });
...
const attachClientEvents = (c) => {
  ...
  c.on("authenticated", () => io.emit("status", { ready: false, step: "authenticated" }));
  c.on("ready", async () => {
  ...
  const handleIncoming = async (msg) => {
    let chatId = msg.from, isGroup = false;
    try { const chat = await msg.getChat(); chatId = chat?.id?._serialized || chatId; isGroup = !!chat?.isGroup; } catch {}
    ...
    try {
      if (!mapped.fromMe && !isGroup && isReady) {
        ...
        if (nowSec() - last >= GREET_COOLDOWN_SEC && !pendingGreeting.has(chatId)) {
          pendingGreeting.add(chatId);
          setTimeout(async () => {
            try {
              await client.sendMessage(chatId, "Здравствуйте! Мы получили ваше сообщение и скоро ответим.");
              saveGreetTs(chatId, nowSec());
            } catch {}
            finally { pendingGreeting.delete(chatId); }
          }, GREET_DELAY_MS);
        }
      }
    } catch {}
    ...
    try {
      ...
      if (ix >= 0) {
        ...
        atomicWrite(chatsFile(), { results: sortChats(arr), savedAt: Date.now() });
        io.emit("chat-patch", { id: rec.id, last: rec.last, lastTs: rec.lastTs, unreadCount: rec.unreadCount, status: rec.status, isLead: rec.isLead });
      } else {
        const status = (!isGroup && !mapped.fromMe) ? "lead" : "client";
        const rec = { id: chatId, name: `+${String(chatId).replace(/\D/g, "")}`, isGroup, unreadCount: mapped.fromMe ? 0 : 1, pinned: false, archived: false, last: lastVal, lastTs: ts, isLead: status === "lead", status };
        atomicWrite(chatsFile(), { results: sortChats([rec, ...arr]), savedAt: Date.now() });
        io.emit("chat-patch", rec);
        if (!isGroup && !mapped.fromMe) pushEventUniqueMonthly("lead_new", chatId, ts);
      }
    } catch {}
  };
...
const initialSync = async () => {
  ...
  const mapped = chats
    ...
    .map((c) => {
      ...
      return { id, name, isGroup: !!c.isGroup, unreadCount: c.unreadCount || 0, pinned: !!c.pinned, archived: !!c.archived, last: "", lastTs: 0, isLead: false, status: "client" };
    });
  ...
  try {
    ...
    for (const x of mapped) {
      ...
      if (old) {
        ...
        x.unreadCount = typeof old.unreadCount === "number" ? old.unreadCount : x.unreadCount;
...
io.on("connection", (socket) => {
  socket.emit("status", { ready: isReady, step: isReady ? "ready" : (lastQR ? "qr" : "init") });
  if (lastQR) socket.emit("qr", { dataUrl: lastQR });
});

// ===== REST =====
app.get("/status", (req, res) => res.json({ ready: isReady, step: isReady ? "ready" : (lastQR ? "qr" : "init"), version: "stable-fallback-1" }));
app.get("/qr", (req, res) => lastQR ? res.json({ dataUrl: lastQR }) : res.status(404).json({ error: "NO_QR" }));

app.get("/chats", (req, res) => {
  try {
    const scope = String(req.query.scope || "").toLowerCase();
    const statusQ = String(req.query.status || "").toLowerCase();
    const onlyLeads = scope === "leads" || req.query.lead === "1" || statusQ === "lead";
    const onlyClients = scope === "clients" || statusQ === "client";
    const onlyDeclined = scope === "declined" || statusQ === "declined";

    const stored = safeRead(chatsFile(), { results: [] });
    const arr = (stored.results || []).map(x => ({ ...x, status: x.status || (x.isLead === true ? "lead" : "client") }));

    let base = arr;
    if (onlyLeads) base = arr.filter(x => (x.status || "lead") === "lead");
    else if (onlyClients) base = arr.filter(x => (x.status || "client") === "client");
    else if (onlyDeclined) base = arr.filter(x => (x.status || "") === "declined");
...
app.post("/refresh-chats", async (req, res) => {
  try { await initialSync(); const stored = safeRead(chatsFile(), { results: [] }); res.json({ ok: true, count: (stored.results || []).length }); }
  catch { res.status(500).json({ error: "REFRESH_FAILED" }); }
});
  ...
  try {
    ...
    try {
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        if (messages.length - i <= MEDIA_LAST && m.hasMedia && ["image","sticker","audio","ptt","video","document"].includes(mm.type)) {
          try { const media = await m.downloadMedia(); if (media) mm.media = { mimetype: media.mimetype, data: media.data, filename: m._data?.filename || null }; } catch {}
        }
        mapped.push(mm);
      }
...
app.post("/mark-seen", async (req, res) => {
  ...
  let live = false;
  if (isReady) {
    try { const chat = await client.getChatById(String(id)); await chat.sendSeen(); live = true; } catch {}
  }
  try {
    const stored = safeRead(chatsFile(), { results: [] });
    const arr = stored.results || [];
    const ix = arr.findIndex(x => x.id === id);
    if (ix >= 0) { arr[ix].unreadCount = 0; atomicWrite(chatsFile(), { results: sortChats(arr), savedAt: Date.now() }); io.emit("chat-patch", { id, unreadCount: 0 }); }
  } catch {}
...
app.post("/send", async (req, res) => {
  try {
    ...
    const options = {}; if (quotedMessageId) options.quotedMessageId = quotedMessageId;
...
app.post("/send-location", async (req, res) => {
  try {
    if (!isReady) return res.status(409).json({ error: "WHATSAPP_NOT_READY" });
    const target = normalizeTarget(req.body || {});
    const { latitude, longitude, description, quotedMessageId } = req.body || {};
    if (!target || typeof latitude !== "number" || typeof longitude !== "number") return res.status(400).json({ error: "BAD_REQUEST" });
    const options = {}; if (quotedMessageId) options.quotedMessageId = quotedMessageId;
    const sent = await client.sendMessage(target, new Location(latitude, longitude, description || ""), options);
    ...
    res.json({ ok: true });
  } catch { res.status(500).json({ error: "SEND_LOCATION_FAILED" }); }
});
...
const statusHandler = (req, res) => {
  try {
    ...
    if (!chatId || !["lead","client","declined"].includes(String(status))) return res.status(400).json({ error: "BAD_REQUEST" });
    ...
    const prev = arr[ix].status || (arr[ix].isLead === true ? "lead" : "client");
    ...
    const nowTs = nowSec();
    if (prev !== "client" && status === "client") pushEventUniqueMonthly("client_new", id, nowTs);
    if (prev !== "declined" && status === "declined") pushEventUniqueMonthly("decline", id, nowTs);
...
const flagHandler = (req, res) => {
  try {
    ...
    if (!chatId || typeof isLead !== "boolean") return res.status(400).json({ error: "BAD_REQUEST" });
    const id = sanitizeId(chatId);
    const stored = safeRead(chatsFile(), { results: [] });
    const arr = stored.results || [];
    const ix = arr.findIndex(x => x.id === id);
    ...
    atomicWrite(chatsFile(), { results: sortChats(arr), savedAt: Date.now() });
    io.emit("chat-patch", { id, isLead: arr[ix].isLead, status: arr[ix].status });

    const nowTs = nowSec();
    if (prev !== "client" && arr[ix].status === "client") pushEventUniqueMonthly("client_new", id, nowTs);

    res.json({ ok: true, id, isLead: arr[ix].isLead, status: arr[ix].status });
  } catch { res.status(500).json({ error: "FLAG_FAILED" }); }
};
...
app.get("/analytics", (req, res) => {
  try {
    const from = req.query.from ? new Date(req.query.from + "T00:00:00").getTime()/1000 : 0;
    const to = req.query.to ? (new Date(req.query.to + "T23:59:59").getTime()/1000) : nowSec();
    const a = readAnalytics();
    const inRange = (ts) => Number(ts)>=from && Number(ts)<=to;
    let leadsNew = 0, clientsNew = 0, declines = 0;
  ...
  } catch { res.status(500).json({ error: "ANALYTICS_FAILED" }); }
});

app.post("/logout", async (req, res) => {
  try {
    isReady = false; io.emit("status", { ready: false, step: "logout" });
    try { await client.logout(); } catch {}
    try { await client.destroy(); } catch {}
    try { await fsp.rm(SESS_PATH, { recursive: true, force: true }); } catch {}
    lastQR = null; await bootClient();
    res.json({ ok: true });
